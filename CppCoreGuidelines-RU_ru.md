...
* **lifetime**: отсутствие утечек (пропуст вызова `delete` или многократное `delete`) и отсутствие доступа к недопустимым объектам (разыменование `nullptr`, использование висячей ссылки).
...
* `"tag"` - это строковой литерал с ссылкой на имя элемента, в котором отображается применяемое правило (например, для [C.134](#Rh-public) это "Rh-public"),
...
Лучший подход заключается в том, чтобы четко указать что подразумевается под double (новая скорость или дельта на старой скорости?) и используемую единицу измерения:
...
##### Плохой пример
    ...
    void g(int n)
    {
        // плохо: the number of elements is not passed to f()
        f(new int[n]);
    }
...
We can of course pass the number of elements along with the pointer:
    ...
    void g2(int n)
    {
        f2(new int[n], m);  // плохо: a wrong number of elements can be passed to f()
    }
...
The standard library resource management pointers fail to pass the size when they point to an object:
    ...
    void g3(int n)
    {
        f3(make_unique<int[]>(n), m);    // плохо: pass ownership and size separately
    }
...
How do we transfer both ownership and all information needed for validating use?
    ...
    unique_ptr<int[]> f6(int n)    // плохо: loses n
    {
    ...
    owner<int*> f7(int n)    // плохо: loses n and we might forget to delete
    {
...
##### Пример
    ...
    void increment1(int* p, int n)    // плохо: error-prone
    {
...
##### Плохой пример
    ...
    void f(char* name)
    {
        ...
        // ...
        if (something) return;   // плохо: if something == true, a file handle is leaked
        // ...
        fclose(input);
    }
...
[Pass pointers to single objects (only)](#Ri-array) and [Keep pointer arithmetic simple](#Res-ptr).
* <a name="Pro-boundsl-the-standard-library"></a>Bounds.4: Don't use standard-library functions and types that are not bounds-checked:
